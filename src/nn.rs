...
fn train_batch(
    ...
    // println!("---------- Batch Info ----------");
    // println!("batch_output => {:?}", output);
    // println!("batch err => {:?}", err);
    println!("batch err per => {:?}", err_per);
    // println!("network_weights => {:?}", network_weights);
    // println!("network_biases => {:?}", network_biases);
...
mod test_nn {
    ...
    fn test_forward_pass_mismatched_weights_biases() {
        ...
        let mut weights = vec![vec![vec![0.1, 0.2], vec![0.3, 0.4]]];
        let mut biases = vec![
            vec![vec![0.1, 0.2], vec![0.3, 0.4]],
            vec![vec![0.5, 0.6], vec![0.7, 0.8]],
        ];
        forward_pass(&inputs, &mut weights, &mut biases);
    }
    ...
    fn test_forward_pass() {
        ...
        let mut biases = vec![vec![vec![0.1, 0.2]], vec![vec![0.3, 0.4]]];
        let (output, layer_outputs) = forward_pass(&inputs, &mut weights, &mut biases);
    ...
    fn test_train_convergence() {
        ...
        let targets = vec![vec![1.0], vec![2.0], vec![3.0], vec![4.0]];
